...
| D-52 | "The AI noticed…" | Shown only on a reshoot or reject decision, never on an approved Assessment, in V2 (V2-Q20 narrowed). | **operator-decided, V2-25 review, 2026-09-26** |
...
| `reshoot` (Expert) | The Expert's note, the **"AI suggestion"** feedback (strengths, gaps and `feedbackEn`/`feedbackSw` in the UI language), "The AI noticed…" Observations without timestamps (D-52), and **Record again** |
| `approved` | The Badge line "Verified by Smart Fundis — <Trade>: <Task> · <date>", then the Expert's note, then **"AI suggestion"** feedback: strengths, gaps and `feedbackEn`/`feedbackSw` in the UI language. **No "The AI noticed…" list on an approved Assessment** in V2 (D-52). |
| `rejected` | The Expert's note, the AI feedback, "The AI noticed…" Observations without timestamps (D-52), and **Appeal** (once) |
...
dashboard.fundiVerifications({ paginationOpts }) (query, requireFundi) → PaginationResult<{
  ...
  aiFeedback: { strengths: string[]; gaps: string[]; text: string;
                noticed: { item: string; result: "yes" | "no" | "unclear" }[] | null  // only for reshoot or reject (D-52)
              } | null;                              // only after a decision
...
| Observations | only after a **reshoot or reject** decision, as "The AI noticed…"; never on `approved` (D-52, V2-Q20 narrowed) | whole-second timestamp chip → jumps the player; "--:--" ("no timestamp") if missing; `unclear` = "The AI couldn't tell" | yes | never |
| Safety flags | nothing before the decision (D-50); after a reshoot or reject, "needs a closer look", never an accusation | first; the queue shows only the "Safety check" marker | yes | never |
...
All copy is in `messages/en.json` and `messages/sw.json` (D-29), with a native-speaker check on Kiswahili (R-20). Kiswahili strings in this spec are drafts.
...
| "The AI noticed…" after every decision (V2-Q20) | this spec's first draft | Only on reshoot or reject, never on `approved`. | D-52 |
...
| V2-Q20 | Show AI Observations to the Fundi after the Expert decides? | **Answered, narrowed (D-52):** only after a **reshoot or reject** decision, as "The AI noticed…", never on an approved Assessment, in V2. |
...
- "The AI noticed…" on approved Assessments (D-52); any Verdict hint to a Fundi before the Expert decides (D-50)
