...
import { TRADE_CATALOGUE, TRADE_SLUGS } from "@convex/lib/trades";
...
describe("TradeCatalogue messages stay in sync with convex/lib/trades.ts", () => {
  ...
  it("uses the stored English name as the display name", () => {
    for (const trade of TRADE_CATALOGUE) expect(catalogue[trade.slug].name, trade.slug).toBe(trade.name);
  });
  ...
  it("labels every type of work the catalogue uses", () => {
    const used = new Set(TRADE_CATALOGUE.map((trade) => trade.category));
