...
| D-11 | `web` is the only npm workspace. `convex/` stays at the root as a plain Convex functions folder, with `convex` and `typescript` in the root `package.json`. A workspace package inside the functions folder would let a `convex/node_modules` get bundled (R-11). Amends the spec's "npm workspaces for `web` and `convex`". | #2 code review |
