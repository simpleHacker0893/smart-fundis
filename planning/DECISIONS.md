...
| D-14 | Protected pages are guarded twice. The proxy redirects signed-out visitors using a plain pathname check (`web/lib/auth-routes.ts`), and **every protected page calls `auth.protect()` itself**, which is the real guard. Clerk advises never relying on the proxy alone, and it now discourages `createRouteMatcher`. | #3 review |
