...
## First, every time
1. Read `AGENTS.md`, `docs/PRD.md` §6 (the current phase's acceptance criteria), the latest `docs/handoff/` file, and the current `docs/prompts/` task.
...
- **i18n:** switch to `sw` and check that no English strings remain on the page and that the choice persists after reload.
